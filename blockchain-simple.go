...
func handlePost(w http.ResponseWriter, r *http.Request) {
	...
	codeNo, err1 := strconv.Atoi(params["CodeNo"])
	newProduct.CodeNo = codeNo
	cost, err2 := strconv.Atoi(params["Cost"])
...
func updateAccount(p Product, snapshot int) { // AFTER location update. VERY VERY IMP "AFTER"
	...
	outFile := *accsDir + "/out-" + strconv.Itoa(newLocation) + ".gob"
	oldLocation := newLocation - 1
	inFile := *accsDir + "/in-" + strconv.Itoa(oldLocation) + ".gob"

	var outAcc []Entry
	gobCheck(readGob(&outAcc, outFile)) // ; fmt.Printf("\x1b[32m%s\x1b[0m> ", spew.Sdump(outAcc))
	var inAcc []Entry
	gobCheck(readGob(&inAcc, inFile)) // ; fmt.Printf("\x1b[32m%s\x1b[0m> ", spew.Sdump(inAcc))
	...
	outE := e
	outE.Party = oldLocation
	outE.Direction = "out"
	outAcc = append(outAcc, outE)
	inE := e
	inE.Party = newLocation
	inE.Direction = "in"
	inAcc = append(inAcc, inE)
...
func genRandPdt() (string, int) {
	...
	randChoice := genRandInt(len(PdtMap), 0)
	i := 0
